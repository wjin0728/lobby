...
namespace JustAssets.TerrainUtility
{
    ...
    /// </summary>
    public sealed partial class SplitTerrain : IHandler<Terrain>
    {
        ...
        private float _blendStrengthAtSeems = 0.6f;
            ...
            set
            {
                switch (index)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        _blendStrengthAtSeems = (float) value;
                        break;
                }
            }
        }
        ...
        private static void SplitControlTexture(TerrainData targetTerrainData, int piecesPerAxis, int sliceIndex,
            TerrainData sourceTerrainData, float[,,] sourceControlTextures)
        {
            ...
            for (var layerIndex = 0; layerIndex < sourceTerrainData.alphamapLayers; layerIndex++)
            {
                for (var x = 0; x < targetControlTextureResolution; x++)
                {
                    ...
                    for (var y = 0; y < targetControlTextureResolution; y++)
                    {
                        ...
                        var ph = BilinearInterpolator.Interpolate(sourceControlTextures, xPos, yPos, layerIndex, sourceControlTextureResolutionMinus1);
        ...
        private void SmoothSeems(float[,,] sourceControlTextures, int piecesPerAxis)
        {
            ...
            for (int x = 1; x < piecesPerAxis; x++)
            {
                ...
                for (int yPos = 0; yPos < height; yPos++)
                {
                    for (int i = 0; i < layerCount; i++)
                    {
                        ...
                        var mix0 = Mathf.Lerp(c1, c0, _blendStrengthAtSeems);
                        var mix1 = Mathf.Lerp(c0, c1, _blendStrengthAtSeems);
