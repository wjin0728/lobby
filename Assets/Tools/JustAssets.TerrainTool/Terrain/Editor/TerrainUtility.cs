...
namespace JustAssets.TerrainUtility
{
    public class TerrainUtility : EditorWindow
    {
        ...
        private readonly Version _version = new Version(2, 2, 1);
            ...
            if (activeOption != null)
            {
                ...
                switch (_operation)
                {
                    case Operation.SplitTerrain:
                        ...
                        var tooltip = "100% means off, 50% percent will evenly blend the pixel at one side of " +
                                      "the border with the one on the other side of the border. Default is 60% keeping " +
                                      "the original a bit stronger while blending 40% of the other tile in. (0% would " +
                                      "swap the pixels at the border) This is all is required due to a missing " +
                                      "implementation of alphamap neighboring inside of the Unity Engine.";
                        handler[3] = EditorGUILayout.Slider(new GUIContent("Blend Strength at Seems", tooltip), ((float)handler[3]) * 100,0,100) / 100;
